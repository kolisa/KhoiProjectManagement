...
namespace KhoiProjectManagementApi.Services
{
    public class IdeaService : IIdeaService
    {
        ...
        public IdeaService(ProjectManagementContext context, INotificationService notificationService, IEmailService emailService, IConfiguration configuration)
        {
            _context = context;
        ...
        public async Task<IdeaDto?> ConvertToProjectAsync(int id, int callerId)
        {
            ...
            await _context.SaveChangesAsync(); // assigns project.Id
        ...
        public async Task<IdeaCommentDto?> AddCommentAsync(int ideaId, CreateIdeaCommentDto dto, ClaimsPrincipal caller)
        {
            ...
            var saved = await _context.IdeaComments.Include(c => c.Author).FirstAsync(c => c.Id == comment.Id);
        ...
        {
            var comment = await _context.IdeaComments.FirstOrDefaultAsync(c => c.Id == commentId);
            if (comment == null)
                return false;
        ...
        public async Task<List<IdeaAttachmentDto>?> GetAttachmentsAsync(int ideaId)
        {
            ...
            var attachments = await _context.IdeaAttachments
        ...
        public async Task<IdeaAttachmentDto?> UploadAttachmentAsync(int ideaId, IFormFile file, ClaimsPrincipal caller)
        {
            ...
            _context.IdeaAttachments.Add(attachment);
            await _context.SaveChangesAsync();

            var saved = await _context.IdeaAttachments
                .Include(a => a.Uploader)
                .Include(a => a.Annotations)
        ...
        {
            var attachment = await _context.IdeaAttachments.FirstOrDefaultAsync(a => a.Id == attachmentId);
            if (attachment == null)
                return null;
        ...
        public async Task<bool> DeleteAttachmentAsync(int attachmentId, ClaimsPrincipal caller)
        {
            ...
            _context.IdeaAttachments.Remove(attachment);
            await _context.SaveChangesAsync();
            return true;
        }
        ...
        {
            var attachmentExists = await _context.IdeaAttachments.AnyAsync(a => a.Id == attachmentId);
            if (!attachmentExists)
                return null;

            var annotations = await _context.IdeaAttachmentAnnotations
                .Include(a => a.Author)
        ...
        public async Task<IdeaAttachmentAnnotationDto?> AddAnnotationAsync(int attachmentId, CreateIdeaAttachmentAnnotationDto dto, ClaimsPrincipal caller)
        {
            ...
            _context.IdeaAttachmentAnnotations.Add(annotation);
            await _context.SaveChangesAsync();

            var saved = await _context.IdeaAttachmentAnnotations.Include(a => a.Author).FirstAsync(a => a.Id == annotation.Id);
            return MapAnnotation(saved);
        }

        public async Task<bool> DeleteAnnotationAsync(int annotationId, ClaimsPrincipal caller)
        {
            var annotation = await _context.IdeaAttachmentAnnotations.FirstOrDefaultAsync(a => a.Id == annotationId);
            if (annotation == null)
                return false;
            ...
            _context.IdeaAttachmentAnnotations.Remove(annotation);
            await _context.SaveChangesAsync();
            return true;
        }
