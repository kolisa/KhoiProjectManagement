        ...
        <div key={i} className="group relative rounded hover:bg-blue-50/50 -mx-2 px-2">
          ...
          {canWrite && (
            <button
              ...
              className="opacity-0 group-hover:opacity-100 absolute -left-6 top-1 text-gray-300 hover:text-blue-600 transition-opacity"
          ...
          {(commentsByBlock[i] || []).length > 0 && (
            <div className="not-prose ml-2 my-2 space-y-1 border-l-2 border-blue-200 pl-3">
              {commentsByBlock[i].map((c) => (
                <div key={c.id} className="text-xs bg-blue-50 rounded p-2 flex justify-between items-start">
                  <div>
          ...
          {commentingIndex === i && (
            <div className="not-prose ml-2 my-2 flex space-x-2">
              ...
              />
              <button onClick={() => handleSubmit(i)} className="text-xs bg-blue-600 text-white px-2 py-1 rounded hover:bg-blue-700">
                Add
              </button>
              <button onClick={() => { setCommentingIndex(null); setDraftBody(''); }} className="text-xs text-gray-500 hover:text-gray-700">
                Cancel
              </button>
            </div>
          )}
